...
export const generateInstallation = async (data: {
  ...
  if (client?.protocol === 'openid-connect') {
    return oidcGenerateInstallation({ kcAdminClient, realm: realm!, client, authServerUrl, authType });
  } else {
    return samlGenerateInstallation({ kcAdminClient, realm: realm!, client: client!, authServerUrl });
  }
};

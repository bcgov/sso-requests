...
describe('Search all requests', () => {
  ...
  let firstId: number, secondId: number, firstClientId: string, secondClientId: string;
  ...
  it('Can search by project id', async () => {
    let result = await getListOfIntegrations({ searchField: ['id'], searchKey: firstId.toString() });
    expect(result.body.count).toBe(1);
    expect(result.body.rows[0].id).toBe(firstId);

    result = await getListOfIntegrations({ searchField: ['id'], searchKey: secondId.toString() });
    expect(result.body.count).toBe(1);

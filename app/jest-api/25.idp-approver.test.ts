...
jest.mock('@app/utils/bcsc-client', () => {
  const original = jest.requireActual('@app/utils/bcsc-client');
  return {
    ...original,
...
describe('IDP Approver', () => {
  ...
  it('BCeID approver can edit/approve/delete owned integrations', async () => {
    ...
    const requests = await getRequestsForAdmins();
    const createdReq = requests.body.rows.find((row: any) => row.projectName === 'bceid-approver');
    expect(requests.status).toEqual(200);
  ...
  it('Social approver can edit/approve/delete owned integrations', async () => {
    ...
    const requests = await getRequestsForAdmins();
    const createdReq = requests.body.rows.find((row: any) => row.projectName === 'social-approver');
    expect(requests.status).toEqual(200);
    expect(createdReq).toBeTruthy();

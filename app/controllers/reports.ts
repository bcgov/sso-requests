...
import { difference, map, partition } from 'lodash';
...
export const getDataIntegrityReport = async () => {
  ...
  const integrationByAccountType = partition(results, function (o: any) {
  ...
  for (let environment of ['dev', 'test', 'prod']) {
    ...
    report[environment]['null-client-ids'] = integrationByAccountType[1]
      .filter((c: any) => c.environments.includes(environment) && !c.client_id)
      .map((c: any) => {
        return {
          id: c.id,
          status: c.status,
    ...
    if (environment === 'prod') {
      integrationByAccountType[0].map((c: any) => {
    ...
    report[environment]['missing-from-keycloak'] = map(cssClients, function (c) {
      const kcClient: any = results.find((kc: any) => kc?.client_id === c);
      return {
        client: c,

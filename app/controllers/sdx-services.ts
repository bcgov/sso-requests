...
export const createSdxRequest = async (session: Session, requestId: number, sdxRequestData: any) => {
  if (!sdxRequestData || Object.keys(sdxRequestData).length === 0 || typeof sdxRequestData !== 'object') {
    throw new Error('SDX request data is invalid or required');
  }

  const transaction = await sequelize.transaction();

  try {
    await models.SdxRequest.create({ requestId, ...sdxRequestData }, { transaction });

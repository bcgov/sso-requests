export const MAX_STEP_ATTEMPTS = Number(process.env.SAGA_MAX_STEP_ATTEMPTS || 5);

/** A claim older than this is assumed to belong to a dead pod and is reclaimed by the next worker. */
export const CLAIM_LEASE_SECONDS = Number(process.env.SAGA_CLAIM_LEASE_SECONDS || 300);

const BASE_DELAY_MS = Number(process.env.SAGA_RETRY_BASE_DELAY_MS || 2_000);
const MAX_DELAY_MS = Number(process.env.SAGA_RETRY_MAX_DELAY_MS || 120_000);

/** Retries shorter than this are re-armed in-process; anything longer waits for the cron tick. */
export const IN_PROCESS_RETRY_CEILING_MS = Number(process.env.SAGA_IN_PROCESS_RETRY_CEILING_MS || 60_000);

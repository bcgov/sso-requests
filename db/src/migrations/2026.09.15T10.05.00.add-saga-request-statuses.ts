export const name = '2026.09.15T10.05.00.add-saga-request-statuses';

// `processing`  - saga is actively applying the integration to Keycloak.
// `compensating` - a step failed permanently and the saga is rolling back in reverse order.
...
// Postgres cannot drop a single enum label, so the type is rebuilt without the saga values. Any
// row still parked on a saga status is collapsed back to `submitted` first.

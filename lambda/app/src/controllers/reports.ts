...
export const getDatabaseTable = async (table: string, orderBy: string) => {
  if (table == 'Requests') {
    ...
    const [results] = await sequelize.query(`
    SELECT id, created_at, updated_at, request_id, event_code, idir_userid, details::text, idir_user_display_name FROM ${table} ORDER BY ${orderBy}, created_at desc limit 5000;
    `);
    return results;
  } else {
    const [results] = await sequelize.query(`
